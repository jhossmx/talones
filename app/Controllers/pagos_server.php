...
class Pagos extends BaseController
{
    ...
    public function index()
    {
        ...
        $data['js'] = ['pagos/pagos'];
        //$data['quincenas'] = $this->model->findAll();
        ...
        //echo print_r($data['pagos']);exit;
        //echo print_r($data['quincenas']);exit;
        ...
        if (!$myFile) {
            ...
            if ($rarFile = $this->request->getFiles()) 
            {
                foreach($rarFile['file-rar'] as $file) 
                {
                    ...
                    $fileXmlNme = $this->getZipFile($extractor, $file);
                    //echo $fileXmlNme;"<br>";
                    ...
                    if($fileXmlNme != "") 
                    {
                        ...
                        if (count($info) >= 3) 
                        {
                            $_anioFile = $this->modelPagos->getIdAnio(substr($info[4], 0, 4));
                            $_quincenaFile = (int) (substr($info[4], 4, 2));
    ...
    public function getZipFile($extractor, $file)
    {
        $myFile = "" ;
        if ($file->isValid() && ! $file->hasMoved()) {
            ...
            $pathFile = WRITEPATH . 'uploads'."//";
            chmod($pathFile.$FileNameRar, 0755);
            
            //se descomprime el archivo
            $extract = $extractor->extract($pathFile.$FileNameRar, $pathFile);
            if($extract)
            {
                ...
                $FileNameXml = $path."CFDI_Comprobante_".strtoupper(trim($info[2]))."_".trim($info[1]).".xml";
                $myFile = $FileNameXml;
            }   
            //es para borrar el archivo rar que estoy analizando 
            unlink( $pathFile.$FileNameRar );
        }
        return $myFile;
    }
    ...
    public function filtrar()
    {
        ...
        if (count($data) > 0) {
            foreach ($data as $row) {
                ...
                echo '<tr class="fs14" style="color:' . $color . '">';
                echo '<td class="text-bold-500">' . $row->anio . ' </td>';
                echo '<td>' . $row->quincena . '</td>';
                echo '<td>' . $row->nomina . '</td>';
                echo '<td>' . $row->tipo . '</td>';
                echo '<td class="text-bold-500">' . $row->concepto . '</td>';
                echo '<td title="'.$row->descripcion.'">' . ((strlen($row->descripcion) > 65 ) ? substr($row->descripcion,0, 64) : $row->descripcion ) . '</td>';
                echo '<td>$' . number_format($row->importe, 2) . '</td>';
                echo '</tr>';
            }
        } else {
            echo '<tr>';
            echo '<td  class="text-bold-500" colspan="7">No hay Pagos</td>';
            echo '</tr>';
        }
        //dd($data);
    }
    ...
    private function readXmlFile($anio, $quincena, $myFile)
    {
        ...
        if (count($response) > 0) {
            ...
            //deducciones
            $deduccionesTmp = $response['Complemento']['Nomina']['Deducciones'];
